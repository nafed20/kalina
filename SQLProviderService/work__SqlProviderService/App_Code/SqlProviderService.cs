...
public class SqlProviderService : ISqlProviderService
{
    ...
    public ErrCode DeleteChannel(DAL.Channel channel)
    {
        ErrCode err = SiteProvider.CurrentProvider.DeleteChannel(channel);

        return err;
    }
    public List<DAL.Channel> LoadAllChannels(out ErrCode err)
    {
        List<DAL.Channel> channels = SiteProvider.CurrentProvider.LoadAllChannels(out err);

        return channels;
    }
    public List<DAL.Channel> LoadChannels(int tb_id, out ErrCode err)
    {
        List<DAL.Channel> channels = SiteProvider.CurrentProvider.LoadChannels(tb_id, out err);
    ...
    public List<CdInfo> GetCds(out ErrCode err)
    {
        err = new ErrCode();
        List<CdInfo> cds = SiteProvider.CurrentProvider.GetCds(out err);
    ...
    public ErrCode SaveDirective(int directive_id, string name, string number, string dateFrom, string dateTill, string typeOfDirective, out int id)
    {
        ErrCode errCode = SiteProvider.CurrentProvider.SaveDirective(directive_id, name, number, dateFrom, dateTill, typeOfDirective, out id);
